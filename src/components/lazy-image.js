...
const LazyImage = ({ src, alt, screenWidth, index }) => {
  ...
  const styles = {
    ...
    */
    dynamicDescriptionParagraph: {
      fontSize: `${screenWidth < 600 && screenWidth > 400 ? 'x-large' : screenWidth < 400 ? 'large' : 'xx-large'}`
    }
  ...
  const descriptionElement = () => {
    ...
    return (
      <p className='descriptionParagraph' style={styles.dynamicDescriptionParagraph}>
      {alt.slice(0, 26) + '...'}
      </p>
    )
  }
